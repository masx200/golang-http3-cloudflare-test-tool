...
**超时集中度分析**: 共有 10 次超时，主要集中在IP段 104.26（6 次），可能存在网络路由问题或目标服务器负载过高

**协议协商分析**: 有 15 次失败是因为协议协商失败（protocol: none），说明无法与目标建立HTTP/3或其他现代协议连接

**IP版本分析**: 所有失败的测试都使用IPv4，IPv6连接可能更稳定或目标服务器的IPv6配置更好

**问题主机分析**: 以下主机出现多次失败：104.26.6.112 (6次)，建议重点检查这些主机的网络状态和服务可用性
...
| 284 | bhr01erx45.inzjddnkdz.de5.net | 104.21.73.250 | IPv4 | h2 | ✅ 成功 | 74 | cloudflare |
