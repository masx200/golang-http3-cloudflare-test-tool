...
| 序号 | 主机/域名 | 目标IP | IP版本 | 协议 | 状态码 | 延迟(ms) | 服务器 | 错误信息 |
|------|-----------|--------|--------|------|--------|----------|--------|----------|
| 179 | ********-****-****-****-************.*.*.*.*.*.*.*.*.*.*.*.*.***.**** | Unknown | Unknown | none | N/A | 0 | N/A | DNS解析无结果 |
| 250 | **********.********************.***.** | Unknown | Unknown | none | N/A | 0 | N/A | DNS解析无结果 |
| 324 | **********.**********.***.*** | Unknown | Unknown | none | N/A | 0 | N/A | DNS解析无结果 |
...
**协议协商分析**: 有 170 次失败是因为协议协商失败（protocol: none），说明无法与目标建立HTTP/3或其他现代协议连接

**IP版本分析**: IPv4失败 3 次，IPv6失败 164 次，两种协议都存在问题

**问题主机分析**: 以下主机出现多次失败：moura.ns.cloudflare.com (3次), rustam.ns.cloudflare.com (3次), wilson.ns.cloudflare.com (3次)，建议重点检查这些主机的网络状态和服务可用性
